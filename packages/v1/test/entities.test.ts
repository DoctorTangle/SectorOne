...
const WETH = _WAVAX[ChainId.BASE]
  ...
  DECIMAL_PERMUTATIONS.forEach((decimals) => {
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Price:Route.midPrice', () => {
        ...
        expect(
          route.midPrice.toSignificant(5, { groupSeparator: ',' })
        ).toEqual('1,234')
        ...
        expect(
          route.midPrice
            .invert()
            .toSignificant(4, undefined, Rounding.ROUND_DOWN)
        ).toEqual('0.0008103')
        ...
        expect(route.midPrice.toFixed(2, { groupSeparator: ',' })).toEqual(
          '1,234.00'
        )
        ...
        expect(
          route.midPrice.invert().toFixed(7, undefined, Rounding.ROUND_DOWN)
        ).toEqual('0.0008103')
      ...
      describe('Trade', () => {
        ...
        it('TradeType.EXACT_INPUT', () => {
          ...
          expect(trade.executionPrice.quote(inputAmount)).toEqual(
            expectedOutputAmount
          )
          expect(
            trade.executionPrice.invert().quote(expectedOutputAmount)
          ).toEqual(inputAmount)

          expect(trade.nextMidPrice.toSignificant(18)).toEqual(
            '1.38958368072925352'
          )
          expect(trade.nextMidPrice.invert().toSignificant(18)).toEqual(
            '0.71964'
          )
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          ...
          const expectedInputAmount = new TokenAmount(
            tokens[1],
            decimalize(1, tokens[1].decimals)
          )
          const trade = new Trade(
          ...
          expect(trade.executionPrice.quote(expectedInputAmount)).toEqual(
            outputAmount
          )
          expect(trade.executionPrice.invert().quote(outputAmount)).toEqual(
            expectedInputAmount
          )
                ...
                new Pair(
                  ...
                  new TokenAmount(
                    ...
                    decimalize(10, WETH.decimals) +
                      (tokens[1].decimals === 9
                        ? BigInt('30090280812437312')
                        : BigInt('30090270812437322'))
                  ),
                  CHAIN_ID
                )
              ],
