...
describe('Trade', () => {
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('provides best route', () => {
      ...
      expect(result[0].outputAmount).toEqual(
        new TokenAmount(token2, JSBI.BigInt(99))
      )
    ...
    it('insufficient input for one pair', () => {
      ...
      expect(result[0].route.path).toEqual([token0, token2])
      expect(result[0].outputAmount).toEqual(
        new TokenAmount(token2, JSBI.BigInt(1))
      )
    })
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result[0].route.path).toEqual([
        WAVAX[CHAIN_ID],
        token0,
        token1,
        token3
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[1].route.path).toEqual([
        token3,
        token1,
        token0,
        WAVAX[CHAIN_ID]
  ...
  describe('#maximumAmountIn', () => {
    describe('tradeType = EXACT_INPUT', () => {
      ...
      it('returns exact if nonzero', () => {
        expect(
          exactIn.maximumAmountIn(new Percent(JSBI.BigInt(0), JSBI.BigInt(100)))
        ).toEqual(new TokenAmount(token0, JSBI.BigInt(100)))
        expect(
    ...
    describe('tradeType = EXACT_OUTPUT', () => {
      ...
      it('returns slippage amount if nonzero', () => {
        ...
        ).toEqual(new TokenAmount(token0, JSBI.BigInt(156)))
        expect(
          exactOut.maximumAmountIn(
            new Percent(JSBI.BigInt(5), JSBI.BigInt(100))
          )
        ).toEqual(new TokenAmount(token0, JSBI.BigInt(163)))
        expect(
          exactOut.maximumAmountIn(
            new Percent(JSBI.BigInt(200), JSBI.BigInt(100))
          )
        ).toEqual(new TokenAmount(token0, JSBI.BigInt(468)))
      })
    })
  })
  ...
  describe('#minimumAmountOut', () => {
    describe('tradeType = EXACT_INPUT', () => {
      ...
      it('returns exact if nonzero', () => {
        ...
        ).toEqual(new TokenAmount(token2, JSBI.BigInt(69)))
        expect(
          exactIn.minimumAmountOut(
            new Percent(JSBI.BigInt(5), JSBI.BigInt(100))
          )
        ).toEqual(new TokenAmount(token2, JSBI.BigInt(65)))
        expect(
          exactIn.minimumAmountOut(
            new Percent(JSBI.BigInt(200), JSBI.BigInt(100))
          )
        ).toEqual(new TokenAmount(token2, JSBI.BigInt(23)))
      })
    })
    describe('tradeType = EXACT_OUTPUT', () => {
      const exactOut = new Trade(
      ...
      it('returns slippage amount if nonzero', () => {
        ...
        ).toEqual(new TokenAmount(token2, JSBI.BigInt(100)))
        expect(
          exactOut.minimumAmountOut(
            new Percent(JSBI.BigInt(5), JSBI.BigInt(100))
          )
  ...
  describe('#bestTradeExactOut', () => {
    ...
    it('provides best route', () => {
      ...
      expect(result[0].route.path).toEqual([token0, token2])
      expect(result[0].inputAmount).toEqual(
        new TokenAmount(token0, JSBI.BigInt(101))
      )
      expect(result[0].outputAmount).toEqual(
        new TokenAmount(token2, JSBI.BigInt(100))
      )
