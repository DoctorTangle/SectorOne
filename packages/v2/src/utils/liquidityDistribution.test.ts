...
import { ChainId, CurrencyAmount, Token, TokenAmount } from '@sectorone/sdk-core'
...
it('getBidAskDistributionFromBinRange with amount X and Y', () => {
  const activeId = 8376038
  const amountA = CurrencyAmount.ether(ChainId.BASE, parseEther('1'))
...
it('getBidAskDistributionFromBinRange with amount Y only', () => {
  const activeId = 8376038
  const amountA = CurrencyAmount.ether(ChainId.BASE, parseEther('0'))
  const amountB = new TokenAmount(token0, parseEther('0.5'))
